  ...
  fillInTheBlankQuestionGroups.forEach(group => {
    group.questions.forEach(questionObj => {
      ...
      for (let i = 0; i < idMatches.length; i++) {
        let userAnswer = document.getElementById(idMatches[i].substring(4, idMatches[i].length - 1)).value;
        userAnswer = replaceChinesePunctuationToEnglish(userAnswer);
        const correctAnswer = answerArray[i].toLowerCase(); // 正确答案
        ...
        group.questions.forEach((questionObj, input_index) => {
          ...
          if (idMatches) {
            for (let i = 0; i < idMatches.length; i++) {
              let userAnswer = document.getElementById(idMatches[i].substring(4, idMatches[i].length - 1)).value;
              userAnswer = replaceChinesePunctuationToEnglish(userAnswer);
              const correctAnswer = answerArray[i].toLowerCase();
